...
class UserAuth(Model):
    ...
    用户注册信息表：用于登录和找回密码
    """
    id = fields.IntField(pk=True, description='唯一用户id')
    ...
    birth_date = fields.DateField(description="用户出生年月")
    is_student = fields.BooleanField(default=False, description="是否大学生客户")
    is_Academic_certified = fields.BooleanField(description="是否通过学信网认证")
...
class UserSignLog(Model):
    ...
    user = fields.ForeignKeyField("finance.UserAuth", related_name="user_login_log", description="关联的用户")
...
class UserApplication(Model):
    ...
    """
    id = fields.IntField(pk=True)
    user = fields.ForeignKeyField("finance.UserAuth", related_name="user_application", description="关联的用户")
    real_name_verified = fields.BooleanField(default=False, description="用户实名制是否通过核实")
    age = fields.IntField(description="用户年龄")
    is_student = fields.BooleanField(default=False, description="是否大学生客户")
    ...
    network_age_months = fields.IntField(description="用户网龄（月）")
    last_payment_months_ago = fields.IntField(description="用户最近一次缴费距今时长（月）")
    last_payment_amount = fields.FloatField(description="最近一次缴费金额（元）")
    avg_monthly_spending_6_months = fields.FloatField(description="用户近6个月平均消费值（元）")
    current_bill_total = fields.FloatField(description="用户账单当月总费用（元）")
    current_account_balance = fields.FloatField(description="用户当月账户余额（元）")
    has_outstanding_payment = fields.BooleanField(default=False, description="当前是否欠费缴费")
    call_fee_sensitivity = fields.IntField(description="用户话费敏感度")
    contacts_this_month = fields.IntField(description="当月通话交往圈人数")
    is_frequent_mall_visitor = fields.BooleanField(default=False, description="是否经常逛商场的人")
    avg_mall_visits_3_months = fields.IntField(description="近三个月月均商场出现次数")
    ...
    online_shopping_app_usage = fields.IntField(description="当月网购类应用使用次数")
    logistics_app_usage = fields.IntField(description="当月物流快递类应用使用次数")
    finance_app_usage = fields.IntField(description="当月金融理财类应用使用总次数")
    video_app_usage = fields.IntField(description="当月视频播放类应用使用次数")
    airplane_app_usage = fields.IntField(description="当月飞机类应用使用次数")
    train_app_usage = fields.IntField(description="当月火车类应用使用次数")
    travel_info_app_usage = fields.IntField(description="当月旅游资讯类应用使用次数")
    credit_score = fields.FloatField(description="用户信用分")
...
class UserBehavior(Model):
    ...
    """
    id = fields.IntField(pk=True)
    user = fields.ForeignKeyField("finance.UserAuth", related_name="user_behavior", description="关联的用户")
    age = fields.IntField(description="用户年龄")
    bank_cards_count = fields.IntField(description="全国股份制商业银行卡片数")
    remote_transaction_months = fields.IntField(description="近6个月有异地交易的月份数")
    internet_transaction_avg = fields.FloatField(description="近6个月互联网交易笔数均值")
    financial_transaction_months = fields.IntField(description="近6个月有金融类交易的月份数")
    financial_transaction_avg_amount = fields.FloatField(description="近6个月金融类交易额均值")
    max_loan_amount_180_days = fields.FloatField(description="180天内单笔放款金额最大值")
    min_loan_amount_180_days = fields.FloatField(description="180天内单笔放款金额最小值")
